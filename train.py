...
def get_arguments():
    ...
    parser.add_argument('--input_dir', type=str, default=INPUT_DIRECTORY,
...
def main():
    ...
    with tf.name_scope('create_outputs'):
        # Allow silence trimming to be skipped by specifying a threshold near
        # zero.
        silence_threshold = args.silence_threshold if args.silence_threshold > \
                                                      EPSILON else None
        gc_enabled = args.gc_channels is not None
        reader_out = AudioReader(
            args.output_dir,
            coord,
            sample_rate=wavenet_params['sample_rate'],
            gc_enabled=gc_enabled,
            ...
            sample_size=args.sample_size,
            silence_threshold=args.silence_threshold)
        audio_output_batch = reader_out.dequeue(args.batch_size)
        if gc_enabled:
            gc_id_batch_output = reader_out.dequeue_gc(args.batch_size)
        else:
            gc_id_batch_output = None
